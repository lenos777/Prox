# PowerShell script to deploy Prox project to VPS
param(
    [string]$ServerIP = "45.92.173.33",
    [string]$Username = "root"
)

Write-Host "🚀 VPS serverga deploy boshlandi..." -ForegroundColor Green
...
# Execute SSH command
Write-Host "🔗 Serverga ulanish va deploy boshlash..." -ForegroundColor Yellow

try {
    # Use ssh command (requires OpenSSH client on Windows)
    $sshCommands | ssh "$Username@$ServerIP" 'bash -s'
    
    Write-Host ""
    Write-Host "🎉 Deploy muvaffaqiyatli yakunlandi!" -ForegroundColor Green
    Write-Host "🌐 Saytingiz tayyor:" -ForegroundColor Cyan
    ...
    Write-Host "   2. SSL sertifikat o'rnating: ssh root@45.92.173.33 'certbot --nginx -d prox.uz -d www.prox.uz'" -ForegroundColor White
}
catch {
    Write-Host "❌ Xatolik yuz berdi: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host ""
    Write-Host "🔧 Qo'lda ishga tushirish uchun:" -ForegroundColor Yellow
    Write-Host "   ssh root@45.92.173.33" -ForegroundColor White
    Write-Host "   Keyin vps-deploy.sh faylidagi buyruqlarni ketma-ket bajaring" -ForegroundColor White
}